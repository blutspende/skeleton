...
func (as *analysisService) createAnalysisResultsBatch(ctx context.Context, tx db.DbConnection, analysisResultSet AnalysisResultSet) (AnalysisResultSet, error) {
	...
	for i := range analysisResultSet.Results {
		if analysisResultSet.Results[i].DEARawMessageID.UUID != uuid.Nil || analysisResultSet.Results[i].DEARawMessageID.Valid {
			return analysisResultSet, fmt.Errorf("DEA raw message ID is manually set at index %d, while it should be set by skeleton internally", i)
		}
		...
		if analysisResultSet.Results[i].AnalyteMapping.ID == uuid.Nil {
			return analysisResultSet, fmt.Errorf("analyte mapping CerberusID is missing at index: %d", i)
		}
		if analysisResultSet.Results[i].Instrument.ID == uuid.Nil {
			return analysisResultSet, fmt.Errorf("instrument CerberusID is missing at index: %d", i)
		}
	...
	for i := range analysisResultSet.ControlResults {
		if analysisResultSet.ControlResults[i].InstrumentID == uuid.Nil {
			return AnalysisResultSet{}, fmt.Errorf("missing instrument ID on standalone control result at index %d", i)
		}
