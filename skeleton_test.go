...
func TestSkeletonStart(t *testing.T) {
	...
	skeletonApi, err := New(ctx, serviceName, sqlConn, "skeleton")
	if err != nil {
		return
	}
...
func TestSubmitAnalysisRequestsParallel(t *testing.T) {
	...
	skeletonInstance, _ := NewSkeleton(ctx, serviceName, sqlConn, schemaName, migrator.NewSkeletonMigrator(), api, analysisRepository, analysisService, instrumentService, consoleLogService, skeletonManager, cerberusClientMock, deaClientMock, configuration)
...
func TestAnalysisResultsReprocessing(t *testing.T) {
	...
	skeletonInstance, _ := NewSkeleton(ctx, serviceName, sqlConn, schemaName, migrator.NewSkeletonMigrator(), api, analysisRepositoryMock, analysisServiceMock, instrumentService, consoleLogService, skeletonManager, cerberusClientMock, deaClientMock, configuration)
...
type cerberusClientMock struct {
	registerInstrumentFunc           func(instrument Instrument) error
	registerInstrumentDriverFunc     func(serviceName string, apiVersion string, apiPort uint16) error
	sendAnalysisResultBatchFunc      func(analysisResults []AnalysisResultTO) (AnalysisResultBatchResponse, error)
...
func (m *cerberusClientMock) RegisterInstrumentDriver(serviceName string, apiVersion string, apiPort uint16) error {
	if m.registerInstrumentDriverFunc == nil {
		return nil
	}
	return m.registerInstrumentDriverFunc(serviceName, apiVersion, apiPort)
}
