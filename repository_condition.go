...
type ConditionRepository interface {
	...
	GetConditionOperand(ctx context.Context, id uuid.UUID) (ConditionOperand, error)
	GetConditionOperandsMap(ctx context.Context) (map[uuid.UUID]ConditionOperand, error)
	GetConditionsMap(ctx context.Context) (map[uuid.UUID]Condition, error)
	GetNamedConditions(ctx context.Context) ([]Condition, error)
	IsAnalyteUsedAsConditionOperand(ctx context.Context, id uuid.UUID) (bool, error)
	...
	Update(ctx context.Context, condition ConditionOperand) error
	UpdateAnalyteIDsInConstantValues(ctx context.Context, oldAnalyteID uuid.UUID, newAnalyteID uuid.UUID) error
...
func (r *conditionRepository) GetConditionsMap(ctx context.Context) (map[uuid.UUID]Condition, error) {
	query := fmt.Sprintf(`SELECT * FROM %s.sk_conditions where deleted_at IS NULL;`, r.dbSchema)
	rows, err := r.db.QueryxContext(ctx, query)
	if err != nil {
		log.Error().Err(err).Msg(msgGetConditionFailed)
		return nil, ErrGetConditionFailed
	}
	...
	for rows.Next() {
		...
		conditions[dao.ID] = convertDAOToCondition(dao)
	}
...
func (r *conditionRepository) GetConditionOperand(ctx context.Context, id uuid.UUID) (ConditionOperand, error) {
...
func (r *conditionRepository) GetConditionOperandsMap(ctx context.Context) (map[uuid.UUID]ConditionOperand, error) {
	query := fmt.Sprintf(`SELECT * FROM %s.sk_condition_operands where deleted_at IS NULL;`, r.dbSchema)
	rows, err := r.db.QueryxContext(ctx, query)
	if err != nil {
		log.Error().Err(err).Msg(msgGetConditionOperandFailed)
		return nil, ErrGetConditionOperandFailed
	}
	defer rows.Close()
	conditionOperandsMap := make(map[uuid.UUID]ConditionOperand)
	for rows.Next() {
		var dao conditionOperandDAO
		err = rows.StructScan(&dao)
		if err != nil {
		...
		conditionOperandsMap[dao.ID] = convertDAOToConditionOperand(dao)
	}

	return conditionOperandsMap, nil
}
...
func (r *conditionRepository) IsAnalyteUsedAsConditionOperand(ctx context.Context, id uuid.UUID) (bool, error) {
	query := fmt.Sprintf(`SELECT id FROM %s.sk_condition_operands where constant_value = $1::varchar AND deleted_at IS NULL LIMIT 1;`, r.dbSchema)
	rows, err := r.db.QueryxContext(ctx, query, id)
	if err != nil {
		log.Error().Err(err).Msg(msgIsAnalyteUsedAsConditionOperandFailed)
		return false, ErrIsConditionOperandReferencedFailed
	}
	defer rows.Close()
	return rows.Next(), nil
...
func (r *conditionRepository) UpdateAnalyteIDsInConstantValues(ctx context.Context, oldAnalyteID uuid.UUID, newAnalyteID uuid.UUID) error {
	query := fmt.Sprintf(`UPDATE %s.sk_condition_operands SET constant_value = $1, modified_at = timezone('utc', now()) WHERE constant_value = $2;`, r.dbSchema)
	_, err := r.db.ExecContext(ctx, query, newAnalyteID, oldAnalyteID)
	if err != nil {
		log.Error().Err(err).Msg(msgUpdateAnalyteIDsInConditionOperandConstantValuesFailed)
		return ErrUpdateAnalyteIDsInConditionOperandConstantValuesFailed
	}
	return nil
}
...
const (
	...
	msgUpdateAnalyteIDsInConditionOperandConstantValuesFailed = "update analyte IDs in condition operands constant values failed"
...
var (
	...
	ErrUpdateAnalyteIDsInConditionOperandConstantValuesFailed = errors.New(msgUpdateAnalyteIDsInConditionOperandConstantValuesFailed)
