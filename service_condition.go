...
type ConditionService interface {
	...
	GetConditionsMap(ctx context.Context) (map[uuid.UUID]Condition, error)
...
func (s *conditionService) getConditionWithTx(ctx context.Context, tx db.DbConnector, id uuid.UUID) (Condition, error) {
	...
	if condition.Operand1 != nil {
		operand1, err = s.conditionRepository.WithTransaction(tx).GetConditionOperand(ctx, condition.Operand1.ID)
		if err != nil {
			return Condition{}, err
		}
	...
	if condition.Operand2 != nil {
		operand2, err = s.conditionRepository.WithTransaction(tx).GetConditionOperand(ctx, condition.Operand2.ID)
		if err != nil {
			return condition, err
		}
...
func (s *conditionService) GetConditionsMap(ctx context.Context) (map[uuid.UUID]Condition, error) {
	tx, err := s.getTransaction()
	if err != nil {
		return nil, err
	}
	conditionsMap, err := s.getConditionsMapWithTx(ctx, tx)
	if s.externalTx != nil {
		return conditionsMap, err
	}
	if err != nil {
		_ = tx.Rollback()
		return nil, err
...
func (s *conditionService) getConditionsMapWithTx(ctx context.Context, tx db.DbConnector) (map[uuid.UUID]Condition, error) {
	conditions, err := s.conditionRepository.WithTransaction(tx).GetConditionsMap(ctx)
	if err != nil {
		return nil, err
	}
	conditionOperands, err := s.conditionRepository.WithTransaction(tx).GetConditionOperandsMap(ctx)
	if err != nil {
		return nil, err
	}
	conditionTOsMap := make(map[uuid.UUID]Condition)
	for _, condition := range conditions {
		conditionTO := getConditionTO(condition.ID, conditions, conditionOperands)
		if conditionTO != nil {
			conditionTOsMap[conditionTO.ID] = *conditionTO
		}
	}
	return conditionTOsMap, nil
}

func getConditionTO(conditionID uuid.UUID, conditionsMap map[uuid.UUID]Condition, operandsMap map[uuid.UUID]ConditionOperand) *Condition {
	condition, ok := conditionsMap[conditionID]
	if !ok {
		return nil
	}
	if condition.SubCondition1 != nil {
		condition.SubCondition1 = getConditionTO(condition.SubCondition1.ID, conditionsMap, operandsMap)
	}
	if condition.SubCondition2 != nil {
		condition.SubCondition2 = getConditionTO(condition.SubCondition2.ID, conditionsMap, operandsMap)
	}
	if condition.Operand1 != nil {
		if operand, ok := operandsMap[condition.Operand1.ID]; ok {
			condition.Operand1 = &operand
		}
	}
	if condition.Operand2 != nil {
		if operand, ok := operandsMap[condition.Operand2.ID]; ok {
			condition.Operand2 = &operand
		}
	}
	return &condition
}
...
func (s *conditionService) updateCondition(ctx context.Context, tx db.DbConnector, condition Condition, oldCondition Condition) error {
	...
	if condition.Operand1 != nil {
		if condition.Operand1.ID == uuid.Nil {
			...
			//updated operand
			err = s.conditionRepository.WithTransaction(tx).Update(ctx, *condition.Operand1)
			if err != nil {
				return err
			}
		} else {
	...
	if condition.Operand2 != nil {
		if condition.Operand2.ID == uuid.Nil {
			...
			//updated operand
			err = s.conditionRepository.WithTransaction(tx).Update(ctx, *condition.Operand2)
			if err != nil {
				return err
			}
		} else {
	...
	if condition.SubCondition1 != nil {
		...
		err = s.deleteWorkItemConditionRecursively(ctx, s.conditionRepository.WithTransaction(tx), oldCondition.SubCondition1.ID)
		if err != nil {
			return err
		}
		hasChange = true
	...
	if condition.SubCondition2 != nil {
		...
		//deleted subcondition
		err = s.deleteWorkItemConditionRecursively(ctx, s.conditionRepository.WithTransaction(tx), oldCondition.SubCondition2.ID)
		if err != nil {
			return err
		}
		hasChange = true
...
func (s *conditionService) deleteConditionWithTx(ctx context.Context, tx db.DbConnector, id uuid.UUID) error {
	...
	err := s.deleteWorkItemConditionRecursively(ctx, conditionTxRepo, id)
	if err != nil {
		return err
	}
	return nil
...
func (s *conditionService) deleteWorkItemConditionRecursively(ctx context.Context, conditionTxRepo ConditionRepository, id uuid.UUID) error {
	...
	if condition.SubCondition1 != nil {
		err = s.deleteWorkItemConditionRecursively(ctx, conditionTxRepo, condition.SubCondition1.ID)
		if err != nil {
			return err
		}
	}
	...
	if condition.SubCondition2 != nil {
		err = s.deleteWorkItemConditionRecursively(ctx, conditionTxRepo, condition.SubCondition2.ID)
		if err != nil {
			return err
		}
	}
...
func (s *conditionService) getTransaction() (db.DbConnector, error) {
	var tx db.DbConnector
	var err error
	if s.externalTx != nil {
		tx = *s.externalTx
	} else {
		tx, err = s.conditionRepository.CreateTransaction()
	}
	return tx, err
}
...
func createLogicConditionEvaluator(condition Condition, index *int, checkRuleConditionErrors *[]ConditionError, appliedTargets []string) ConditionEvaluatorFunc {
	...
	*index = *index + 1
	...
	return func(operand1AnalysisRequest AnalysisRequest, operand2AnalysisRequest AnalysisRequest, relatedWorkItems []AnalysisRequest) (bool, error) {
		...
		subcondition2Evaluation, err := subCondition2Evaluator(operand1AnalysisRequest, operand2AnalysisRequest, relatedWorkItems)
		if err != nil {
			return false, err
		}
...
func createNotContainsConditionEvaluator(condition Condition) ConditionEvaluatorFunc {
	...
	return func(operand1AnalysisRequest AnalysisRequest, operand2AnalysisRequest AnalysisRequest, relatedWorkItems []AnalysisRequest) (bool, error) {
		res, err := containsEvalFunc(operand1AnalysisRequest, operand2AnalysisRequest, relatedWorkItems)
		return !res, err
	}
}
...
func createNotEqualsConditionEvaluator(condition Condition) ConditionEvaluatorFunc {
	...
	return func(operand1AnalysisRequest AnalysisRequest, operand2AnalysisRequest AnalysisRequest, relatedWorkItems []AnalysisRequest) (bool, error) {
		res, err := equalsEvalFunc(operand1AnalysisRequest, operand2AnalysisRequest, relatedWorkItems)
		return !res, err
	}
}
