...
type MessageOutOrderRepository interface {
	...
	WithTransaction(tx db.DbConnector) MessageOutOrderRepository
}
...
func (r *messageOutOrderRepository) CreateBatch(ctx context.Context, messageOutOrders []MessageOutOrder) ([]uuid.UUID, error) {
	...
	err := utils.Partition(len(messageOutOrders), maxParams/4, func(low int, high int) error {
		...
		_, err := r.db.NamedExecContext(ctx, query, convertMessageOutOrdersToDAOs(messageOutOrders))
		if err != nil {
...
func (r *messageOutOrderRepository) GetBySampleCodesAndRequestMappingIDs(ctx context.Context, sampleCodes []string, requestMappingIDs []uuid.UUID, includePending bool) (map[string]map[uuid.UUID][]MessageOutOrder, error) {
	...
	INNER JOIN infinity.sk_message_out_orders smoo ON smo.id = smoo.message_out_id
		WHERE smoo.sample_code IN (?) AND smoo. IN (?) %s;`, r.dbSchema, filterCondition)
...
func (r *messageOutOrderRepository) GetTestCodesToRevokeBySampleCodes(ctx context.Context, instrumentID uuid.UUID, analysisRequestIDs []uuid.UUID) (map[string][]string, error) {
	...
	query := `SELECT DISTINCT srm.code, smoo.sample_code FROM %schema_name%.sk_message_out smo
				...
				INNER JOIN %schema_name%.sk_message_out_orders_analyis_requests smoor ON smoo.id = smoor.message_out_order_id
				INNER JOIN %schema_name%.sk_request_mappings srm ON srm.id = smoo.request_mapping_id
					WHERE smo.instrument_id = ? AND smo.status = ? AND smoor.analysis_request_id IN (?)
						AND NOT EXISTS (SELECT 1 FROM %schema_name%.sk_message_out_orders_analyis_requests smoor2 
							INNER JOIN %schema_name%.sk_analysis_requests sar ON sar.id = smoor2.analysis_request_id WHERE smoor2.message_out_order_id = smoo.id );`
...
func (r *messageOutOrderRepository) CreateTransaction() (db.DbConnector, error) {
	return r.db.CreateTransactionConnector()
}

func (r *messageOutOrderRepository) WithTransaction(tx db.DbConnector) MessageOutOrderRepository {
	txRepo := *r
	txRepo.db = tx
	return &txRepo
}

func NewMessageOutOrderRepository(db db.DbConnector, dbSchema string, maxRetries int) MessageOutOrderRepository {
