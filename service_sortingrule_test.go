...
func TestGetSortingTargetProgramme1MultiTimeDonorWithCMV(t *testing.T) {
	...
	var err error
	for i := range programme1SortingRules {
		target, err = GetSortingTargetForAnalysisRequestAndCondition([]AnalysisRequest{multiTimeAnalysis1, multiTimeAnalysis2, multiTimeCMV}, programme1SortingRules[i], nil, 0)
		if err == nil {
			break
		}
	}
...
func TestGetSortingTargetProgramme1MultiTimeDonorWithoutCMV(t *testing.T) {
	...
	var err error
	for i := range programme1SortingRules {
		target, err = GetSortingTargetForAnalysisRequestAndCondition([]AnalysisRequest{multiTimeAnalysis1, multiTimeAnalysis2}, programme1SortingRules[i], nil, 0)
		if err == nil {
			break
		}
	}
...
var programme1SortingRules = []SortingRule{
	{
		...
		Priority: 1,
		Condition: &Condition{
			Operand1: &ConditionOperand{Type: Constant, ConstantValue: &str2},
