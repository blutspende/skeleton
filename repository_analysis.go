...
func (r *analysisRepository) createControlResults(ctx context.Context, controlResults []ControlResult) ([]uuid.UUID, error) {
	...
	err := utils.Partition(len(controlResults), controlResultBatchSize, func(low int, high int) error {
		query := fmt.Sprintf(`INSERT INTO %s.sk_control_results(id, sample_code, analyte_mapping_id, instrument_id, expected_control_result_id, is_valid, result, examined_at, created_at)
		VALUES(:id, :sample_code, :analyte_mapping_id, :instrument_id, :expected_control_result_id, :is_valid, :result, :examined_at, :created_at)`, r.dbSchema)
