...
func (r *analysisRepository) UpdateStatusAnalysisResultsBatch(ctx context.Context, analysisResultsToUpdate []AnalysisResult) error {
	...
	args := make([]map[string]interface{}, len(analysisResultsToUpdate))
	for i := range analysisResultsToUpdate {
		args[i] = map[string]interface{}{
			"status": analysisResultsToUpdate[i].Status,
			"id":     analysisResultsToUpdate[i].ID,
		}
	}

	err := utils.Partition(len(analysisResultsToUpdate), analysisResultBatchSize, func(low int, high int) error {
		query := fmt.Sprintf(`UPDATE %s.sk_analysis_results SET status = :status WHERE id = :id;`, r.dbSchema)
		_, err := r.db.NamedExecContext(ctx, query, args[low:high])
		if err != nil {
			log.Error().Err(err).Msg("create analysis result batch failed")
			return err
		}
