...
type MessageOutRepository interface {
	...
	GetMessageOutSampleCodeIDsToDelete(ctx context.Context, limit, offset int) ([]uuid.UUID, error)
...
func (r *messageOutRepository) GetMessageOutSampleCodeIDsToDelete(ctx context.Context, limit, offset int) ([]uuid.UUID, error) {
	query := fmt.Sprintf(`SELECT id FROM %s.sk_message_out_sample_codes WHERE uploaded_to_dea_at IS NOT NULL AND created_at < current_date - '1 MONTH'::INTERVAL ORDER BY created_at DESC LIMIT $1 OFFSET $2;`, r.dbSchema)
