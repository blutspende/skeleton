...
func (r *messageInRepository) GetUnprocessed(ctx context.Context, limit, offset int, cutoffTime time.Time) ([]MessageIn, error) {
	...
	rows, err := r.db.Queryx(ctx, query, instrumentenum.MessageStatusProcessed, r.maxRetries, r.sentDaysBack, cutoffTime, limit, offset)
	if err != nil {
...
func (r *messageInRepository) GetUnprocessedByInstrumentID(ctx context.Context, instrumentID uuid.UUID, limit, offset int) ([]MessageIn, error) {
	query := fmt.Sprintf(`SELECT * FROM %s.sk_message_in 
         							WHERE instrument_id = $1 AND status <> $2 AND retry_count < $3 
         								...
         								ORDER BY created_at DESC LIMIT $5 OFFSET $6;`, r.dbSchema)
	rows, err := r.db.Queryx(ctx, query, instrumentID, instrumentenum.MessageStatusProcessed, r.maxRetries, r.sentDaysBack, limit, offset)
	if err != nil {
		log.Error().Err(err).Msg(msgGetUnprocessedMessageInsFailed)
		return nil, ErrGetUnprocessedMessageInsFailed
	}
...
func (r *messageInRepository) DeleteOldMessageInRecords(ctx context.Context, cleanupDays int, limit int) (int64, error) {
	query := `DELETE FROM %schema_name%.sk_message_in WHERE id IN 
                                      (SELECT smi.id
											FROM %schema_name%.sk_message_in smi
												LEFT JOIN %schema_name%.sk_message_out smo ON smi.id = smo.trigger_message_in_id OR smi.id = smo.response_message_in_id
...
const (
	...
	msgDeleteOldMessageInRecordsFailed = "delete old message in records failed"
