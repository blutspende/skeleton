...
// TODO - consider moving maxRetries and sentDaysBack to receiver level
...
type MessageInRepository interface {
	...
	GetUnprocessedByInstrumentID(ctx context.Context, instrumentID uuid.UUID) ([]MessageIn, error)
...
func (r *messageInRepository) GetUnprocessedByInstrumentID(ctx context.Context, instrumentID uuid.UUID) ([]MessageIn, error) {
	query := fmt.Sprintf(`SELECT * FROM %s.sk_message_in 
         							WHERE instrument_id = $1 AND status <> $2 AND retry_count < $3 
         								AND created_at >= (current_date - make_interval(days := $4) 
         								ORDER BY created_at DESC LIMIT $5 OFFSET $6;`, r.dbSchema)
	rows, err := r.db.QueryxContext(ctx, query, instrumentID, messagestatus.Processed)
	if err != nil {
		log.Error().Err(err).Msg(msgGetUnprocessedMessageInsFailed)
		return nil, ErrGetUnprocessedMessageInsFailed
	}
...
func (r *messageInRepository) GetUnsynced(ctx context.Context, limit, offset int, cutoffTime time.Time) ([]MessageIn, error) {
	...
	rows, err := r.db.QueryxContext(ctx, query, r.maxRetries, cutoffTime, r.sentDaysBack, limit, offset)
	if err != nil {
...
func NewMessageInRepository(db db.DbConnector, dbSchema string, maxRetries, sentDaysBack int) MessageInRepository {
