...
type MessageInRepository interface {
	...
	GetMessageInSampleCodeIDsToDelete(ctx context.Context, limit, offset int) ([]uuid.UUID, error)
...
func (r *messageInRepository) GetMessageInSampleCodeIDsToDelete(ctx context.Context, limit, offset int) ([]uuid.UUID, error) {
	query := fmt.Sprintf(`SELECT id FROM %s.sk_message_in_sample_codes WHERE uploaded_to_dea_at IS NOT NULL AND created_at < current_date - '3 MONTHS' ORDER BY created_at DESC LIMIT $1 OFFSET $2;`, r.dbSchema)
...
func (r *messageInRepository) GetUnsentMessageInSampleCodes(ctx context.Context) (map[uuid.UUID]map[uuid.UUID]string, error) {
	...
	query := fmt.Sprintf(`SELECT id, message_in_id, sample_code FROM %s.sk_message_in_sample_codes WHERE uploaded_to_dea_at IS NULL AND ;`, r.dbSchema)
